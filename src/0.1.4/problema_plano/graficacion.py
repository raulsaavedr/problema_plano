...
def prueba_matrices_cuadradas_acoplamiento(integrandos_matrices_acoplamiento, matrices_acoplamiento_int,\
    ...
    for M in integrandos_matrices_acoplamiento.index:
        ...
        plt.plot(N, matrices_acoplamiento_int.loc[M].stack().loc[:n_dimension,:n_dimension], 'r', label='sol. integrate.quad')
        plt.plot(N, matrices_acoplamiento_sol.loc[M].stack().loc[:n_dimension,:n_dimension], 'b', label='sol. analitica ' + error_t)
        plt.legend(loc='lower right')
...
def graficas_potencial(regiones, potenciales, mesh_regiones, n_dimension):
    ...
    for n_potencial in potenciales.index:
        ...
        plt.title(f"Con nr={n_dimension}- surf del potencial de la {index_reg_actual}-{regiones.loc[index_reg_actual, 'eps']}.")
...
def main():
    ...
    matrices_acoplamiento_int = m_acop.calcular_matrices_acoplamiento_integral(integrandos_matrices_acoplamiento, vectores_valores_eigen.loc[:,0:5], n_dimension=5)
    ...
    prueba_matrices_cuadradas_acoplamiento(integrandos_matrices_acoplamiento, matrices_acoplamiento_int, matrices_acoplamiento_sol, n_dimension=5)
    print("\n\tPrueba de Matrices de cuadradas de acoplamiento con error\n")
    prueba_matrices_cuadradas_acoplamiento(integrandos_matrices_acoplamiento, matrices_acoplamiento_int, matrices_acoplamiento_sol, n_dimension=5, error=2)
    ...
    prueba_potencial(regiones, recursos_potencial, potenciales, potenciales, dimension_mesh, n_dimension)
    print("\n\t\tPrueba de Potenciales con error\n")
    prueba_potencial(regiones, recursos_potencial, potenciales, potenciales_err, dimension_mesh, n_dimension,error=1.05)
    print("\n\t\tPrueba de Flujos sin error\n")
    prueba_flujo(regiones, recursos_flujo, flujos, flujos, dimension_mesh, n_dimension)
    print("\n\t\tPrueba de flujos con error\n")
    prueba_flujo(regiones, recursos_flujo, flujos, flujos_err, dimension_mesh, n_dimension,error=1.05)
    print("\n\t\tSurf de potencial\n")
