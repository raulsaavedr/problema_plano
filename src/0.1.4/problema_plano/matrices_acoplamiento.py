...
def cargar_integrandos_matrices_acoplamiento():
    ...
    se calcularan las matrices cuadradas de acoplamiento. El archivo
    matrices_cuadradas_acoplamiento.csv debe ser definido de la siguiente
    manera:
    ...
    integrandos_matrices_acoplamiento = pd.read_csv(filename)
...
def calcular_matrices_acoplamiento_integral(integrandos_matrices_acoplamiento, vectores_valores_eigen, n_dimension=100):
    """
    Funcion encargada de calcular las matrices de acoplamiento a traves de la
    ...
    Salida:
        * matrices_acoplamiento: DataFrame que contiene todas las matrices cua-
    ...
    # Se crea el dataframe de matrices de acoplamiento de n_dimension*n_dimension
    matrices_acoplamiento = vectores_valores_eigen.reindex(index)
    print("\n\tCalculando matrices cuadradas de acoplamiento:\n\t\tSolucion con quad de scipy\n")
    for M in integrandos_matrices_acoplamiento.index:
        ...
        matrices_acoplamiento.loc[M] = integrar_func_vectorized('integrando_' + tipo_f,xi,xf,vector_valor_eigen_1, desp_1,vector_valor_eigen_2, desp_2).T
    return matrices_acoplamiento

def calcular_matrices_acoplamiento_solucion(integrandos_matrices_acoplamiento, vectores_valores_eigen, n_dimension=100):
    """
    Funcion encargada de calcular las matrices de acoplamiento a traves de la
    ...
    """
    index = pd.MultiIndex.from_tuples([(M, i) for M in integrandos_matrices_acoplamiento.index for i in range(1, n_dimension + 1)])
    # Se crea el dataframe de matrices de acoplamiento
    matrices_acoplamiento = vectores_valores_eigen.reindex(index)
    print("\n\tCalculando matrices cuadradas de acoplamiento:\n\t\t\tSolucion analitica\n")
    for M in integrandos_matrices_acoplamiento.index:
        chr_eigen_1 = integrandos_matrices_acoplamiento.loc[M, 'chr_eigen_1']
        ...
        vector_valor_eigen_2 = vectores_valores_eigen.loc[chr_eigen_2].to_numpy()[:, np.newaxis]
        print(f"* Matriz {M} = {integrandos_matrices_acoplamiento.loc[M, 'calcular_str']}")
        matrices_acoplamiento.loc[M] = eval(tipo_f + '(xi, xf, vector_valor_eigen_1, desp_1, vector_valor_eigen_2, desp_2)').T
    return matrices_acoplamiento
...
def calcular_matrices_acoplamiento_transpuestas(matrices_acoplamiento):
    ...
    return matrices_acoplamiento_transpuestas
...
def main():
    ...
    matrices_acoplamiento_int = calcular_matrices_acoplamiento_integral(integrandos_matrices_acoplamiento, vectores_valores_eigen, n_dimension)
    # Se calcula las matrices con la solucion analitica
    matrices_acoplamiento_sol = calcular_matrices_acoplamiento_solucion(integrandos_matrices_acoplamiento, vectores_valores_eigen, n_dimension)
    # Se calcula las matrices acopladoras transpuestas
    matrices_acoplamiento_trans = calcular_matrices_acoplamiento_transpuestas(matrices_acoplamiento_int)
    print(f"\nSolucion calculada con quad\n{matrices_acoplamiento_int}")
    print(f"\nSolucion calculada con analitica\n{matrices_acoplamiento_sol}")
    print(f"\nDiferencia entre soluciones:\n{matrices_acoplamiento_int - matrices_acoplamiento_sol}")
    print(f"\nMatrices transpuestas de acoplamiento\n{matrices_acoplamiento_trans}")
