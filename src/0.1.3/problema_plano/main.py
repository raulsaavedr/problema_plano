...
# Solo alertas criticas para el paquete numexpr
logging.getLogger("numexpr").setLevel(logging.CRITICAL)
# Se cargan las configuraciones del problema plano
# Si es pasado por argumento: problema_plano y n_dimension
if len(sys.argv) > 1:
    conf.load(problema_plano=sys.argv[1])
    print(f"Ejecutando problema_plano: {sys.argv[1]}")
    if len(sys.argv) > 2:
        try:
            conf.data['vars']['n_dimension'] = int(sys.argv[2])
            print(f"Numero de dimension:{int(sys.argv[2])}")
        except Exception as e:
            raise ValueError(f"\n\nArgs: {sys.argv[0]}: <nombre_problema_plano> <n_dimension>")
else:
    # ejecute este problema plano
    conf.load(problema_plano='ejemplo')

# Se define el nivel de logging del modulo main
logger = logging.getLogger('__main__')
logger.setLevel(conf.get_logging_level('__main__'))
