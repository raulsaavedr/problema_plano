...
def load_vectores_distorsionadores():
    ...
    for i in range(n_vectores_distorsionadores):
        ...
        if int_por_partes == 1:
            print(f"S{i+1}(m)=\u222B((x-x{xf})/(x{xf}-x{xi}))*{tipo_map[int_tp]}({chr_eigen}m*x) de x{xi} a x{xf}")
        else:
...
def load_vectores_transpuestos():
    """\nload_vectores_transpuestos():\n
    Funcion utilizada para llenar el archivo csv: vectores_transpuestos.csv
    Se pide por cada region lo siguiente:
        * chr_vector_t: el nombre del vector transpuesto [A-Z]
        * chr_eigen: vector eigen debe ser una Letra [A-Z]
        * tipo: funcion trigonometrica (1)sin o (2)cos. Debe ser entero
        * x_desp: desplazamiento en x del vector (x-x_desp). Debe ser entero

        La expresion se forma de la siguiente forma:
        vector_transpuesto = tipo(vector_eigen*(x - x_desp))
    """
    print(load_vectores_transpuestos.__doc__)
    lista = [['num_de_region', 'chr_vector_t', 'chr_eigen', 'tipo_f', 'x_desp']]
    n_regiones = conf.data['vars']['n_regiones']
    tpf_map = {1:'sin', 2:'cos'}
    for i in range(n_regiones):
        print(f"\n\t\tDe la region.{i+1}")
        chr_vector_t = input("Inserte el nombre del vector transpuesto:").upper()
        int_tipo_f = int(input("Inserte la funcion trigonometrica del vector transpuesto (1)sin (2)cos:"))
        chr_eigen = input("Inserte el nombre del vector eigen:").upper()
        x_desp = int(input("Inserte el desplazamiento de x (entero):"))
        print(f"{chr_vector_t}\u1D40(x)={tpf_map[int_tipo_f]}({chr_eigen}i(x-x{x_desp}))")
        lista.append([i+1, chr_vector_t, chr_eigen, tpf_map[int_tipo_f], x_desp])
    ...
    with open(filename, 'w') as file:
        ...
        writer.writerows(lista)
    # Se aplican los cambios al diccionario de configuracion data
    conf.data['csv_filled']['vectores_transpuestos'] = True
    conf.update_config_file()
    print(f"El archivo {filename} ha sido creado con exito!")

def load_continuidad():
    """
    load_continuidad():\n
    Funcion utilizada para llenar el archivo csv: continuidad.csv
    Se pide por cada superficie de contacto:
        * Coordenada Y
        * Punto de contacto inicial xi
        * Punto de contacto final xf
        * Numero identificador de la region inferior
        * Numero identificador de la region superior
    
    IMPORTANTE SOLO SE LE PEDIRAN LOS NUMEROS DE REFERENCIA
    TODAS LAS RESPUESTAS DEBEN SER NUMERICAS
    """
    print(load_continuidad.__doc__)
    lista = [['y','xi','xf','region_inferior','region_superior']]
    n_contactos = int(input('Ingrese el numero de regiones de contacto: '))
    for n in range(n_contactos):
        lista.append([])
        lista[n+1].append(input(f'Coordenada Y donde sucede el contacto {n+1}: Y'))
        lista[n+1].append(input(f'Coordenada Xi donde inicia el contacto {n+1}: X'))
        lista[n+1].append(input(f'Coordenada Xf donde finaliza el contacto {n+1}: X'))
        lista[n+1].append('R'+input(f'Identificador de la region Inferior: Reg'))
        lista[n+1].append('R'+input(f'Identificador de la region Superior: Reg'))
    
    filename = 'csv/' + conf.data['env']['path'] + '/continuidad.csv'
    with open(filename, 'w') as file:
        writer = csv.writer(file)
        writer.writerows(lista)
    # Se aplican los cambios al diccionario de configuracion data
    conf.data['csv_filled']['continuidad'] = True
    conf.update_config_file()
    print(f"El archivo {filename} ha sido creado con exito!")
        
def load_fronteras():
    """\nload_fronteras():\n
    Funcion utilizada para llenar el archivo csv: fronteras.csv
    Se pide por cada region lo siguiente:
        * frontera superior
        * frontera inferior
        * frontera derecha
    ...
    las fronteras pueden tener los siguientes valores:
        *No: no hay frontera definida como en el cabio de una region a otra
        *Uno: un potencial de frontera definido en V=1
        *Cero: un potencial de frontera definido en V=0 
        *Compuesta: una frontera compuesta es aquella que se constituye a trozos no continuos
            son de la forma: Potencial-x_inicial-x_final
    """
    print(load_fronteras.__doc__)
    lista = [['arriba', 'abajo', 'derecha', 'izquierda']]
    n_regiones = conf.data['vars']['n_regiones']
    
    tp_map = {0: 'Cero', 1:'Uno', 2:'no', 3:'SIM'}
    
    for i in range(n_regiones):
        print(f"\n\t\tDe la region.{i+1}")
        lista.append([])
        for frontera in ['superior','inferior','derecha','izquierda']:
            cadena = ''
            op = int(input(f"Ingrese la frontera {frontera}: (0)V=0 (1)V=1 (2)Ninguna (3)Simetria (4)Compuesta "))
            
            if op == 4:
                n_segmentos = int(input('Cuantos segmentos tiene la frontera? '))    
                for n in range(n_segmentos):
                    if n != 0: cadena+='/'
                    pot = int(input(f"Ingrese el tipo de potencial del segmento {n+1}: (0)V=0 (1)V=1 (2)Ninguna (3)Simetria "))
                    xi  = input(f"Ingrese el punto x donde inicia el segmento {n+1}: ")
                    xf  = input(f"Ingrese el punto x donde finaliza el segmento {n+1}: ")
                    
                    cadena+='-'.join([tp_map[pot],xi,xf])
            else:
                cadena = tp_map[op]
                
            lista[i+1].append(cadena)
            
            
    filename = 'csv/' + conf.data['env']['path'] + '/fronteras.csv'
    with open(filename, 'w') as file:
        writer = csv.writer(file)
        writer.writerows(lista)
    # Se aplican los cambios al diccionario de configuracion data
    conf.data['csv_filled']['fronteras'] = True
    conf.update_config_file()
    print(f"El archivo {filename} ha sido creado con exito!")
