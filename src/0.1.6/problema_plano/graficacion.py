...
def control_de_continuidad(regiones, potenciales, mesh_regiones, n_dimension):
    ...
    for index in continuidad.index:
        ...
        plt.plot(left_bar, [-2,2],'k')
        plt.plot(right_bar, [-2,2],'k')
        
        filename ='graficas/' + conf.data['env']['path'] +f'/continuidad de potencial/Reg.{R_inf} a la Reg.{R_sup}.svg'
        canvas = FigureCanvas(fig)
        canvas.print_figure(filename)
        plt.close()
...
def graficas_potencial(regiones, potenciales, mesh_regiones, n_dimension):
    ...
    for n_potencial in potenciales.index:
        ...
        ax.plot_surface(x_flat,y_flat,pot,cmap=cm.viridis)
        #ax.view_init(0,-90)
...
def graficar_problema_plano_3D(regiones,z=2):
    ...
    filename = 'graficas/' + conf.data['env']['path'] + "/Problema Plano 3D.svg"
...
def graficar_problema_plano_2D(regiones):
    ...
    filename = 'graficas/' + conf.data['env']['path'] + "/Problema Plano 2D.svg"
