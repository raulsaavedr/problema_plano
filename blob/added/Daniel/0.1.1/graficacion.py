...
def prueba_matrices_cuadradas_acoplamiento(integrandos_matrices_acoplamiento, matrices_acomplamiento_int,\
                                           matrices_acomplamiento_sol, n_dimension=100, error=1):
    """
...
def prueba_potencial(regiones, recursos_potencial, potenciales, potenciales_err, dimension_mesh,\
                     n_dimension=100, error=1):
    """
    Funcion encargada de graficar con matplotlib los potenciales de cada region.
    Equivalente a : f12_V_dV_Prueba()
    Pametros de entrada:
...
def prueba_flujo(regiones, recursos_flujo, flujos, flujos_err, dimension_mesh,\
                 n_dimension=100, error=1):
    """
    Funcion encargada de graficar con matplotlib los flujos de cada region.
    Equivalente a : f12_V_dV_Prueba()
    Pametros de entrada:
...
def graficas_potencial(regiones, potenciales, mesh_regiones, n_dimension):
    ...
    for n_potencial in potenciales.index:
        ...
        #ax.view_init(0,-90)
        plt.savefig(f"graficas/potenciales/surf/Surf" + " " + n_potencial + ".svg", bbox_inches='tight')
        plt.close()
...
def main():
    ...
    mesh_regiones, dimension_mesh = reg.cargar_mesh_regiones(regiones, delta=5)
