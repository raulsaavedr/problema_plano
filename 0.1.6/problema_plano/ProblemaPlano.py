...
"""
class ProblemaPlano:
    def __init__(self, usuario='ejemplo'):
        conf.load(problema_plano=usuario)
        
    def run(self,n=100):
        # Si faltan archivos por cargar del problema plano actual, carguelos
        fill_csv.run()
        # Se cargan las regiones respectivas
        regiones = reg.cargar_regiones(n_dimension=n)  # region.cargarRegiones(2000000)
        ...
        constantes_c = m_gauss.calcular_matriz_gauss(vectores_valores_eigen, matrices_diagonales_valores_eig, matrices_diagonales_hiperbolicas, matrices_acoplamiento_sol,vectores_distorsionadores_sol, matrices_acoplamiento_trans, n_dimension)
...
if __name__ == '__main__':
    ...
    problema = ProblemaPlano('Raul Saavedra')
    problema.run(n=10)
