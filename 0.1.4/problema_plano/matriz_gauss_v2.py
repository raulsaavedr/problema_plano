...
def calcular_matriz_gauss(vectores_valores_eigen, matrices_diagonales_valores_eig,
    ...
    for letra in letras_str:
        ...
        for constante_str in constantes_str:
            logger.debug(letra + ", " + constante_str)
            logger.info(matriz_coeficientes_calcular_str.loc[letra, constante_str])
    ...
    matriz_coeficientes.to_csv('csv/' +  conf.data['env']['path'] + '/salida/matriz_coeficientes_pd.csv')
    vectores_valores_dependientes.to_csv('csv/' +  conf.data['env']['path'] + '/vectores_valores_dependientes_pd.csv', header=True)
...
def main():
    ...
    regiones = reg.cargar_regiones(n_dimension=10)#n_dimension=input("Inserte dimension del problema plano:"))
    ...
    # ordenar_constantes()
    constantes_c = calcular_matriz_gauss(vectores_valores_eigen, matrices_diagonales_valores_eig, matrices_diagonales_hiperbolicas, matrices_acoplamiento_sol,
